...
import com.smart.sso.server.model.secure.EncType;
import com.smart.sso.server.util.secure.CoderUtils;
...
public abstract class BaseSecure {
    ...
    public String encode(byte[] data, EncType encType) {
        ...
        switch (encType) {
            case HEX:
                ...
                break;
            case BASE64:
                d= CoderUtils.encodeBase64(data);
                break;
            case DEFAULT:
                d=new String(data);
                break;
    ...
    public byte[] decode(String data, EncType encType) throws Exception {
        ...
        switch (encType) {
            case HEX:
                ...
                break;
            case BASE64:
                d= CoderUtils.decodeBase64(data);
                break;
            case DEFAULT:
                d=data.getBytes();
                break;
    ...
    public String getEncrypt2Base64(byte[] data) {
        ...
        return pattern.matcher(new String(CoderUtils.encodeBase64(digestData))).replaceAll("");
    }
    ...
    public String getDecrypt2Base64(byte[] data) {
        byte[] digestData = new byte[0];
        try {
            digestData = decrypt(CoderUtils.decodeBase64(new String(data)));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return pattern.matcher(new String(digestData)).replaceAll("");
